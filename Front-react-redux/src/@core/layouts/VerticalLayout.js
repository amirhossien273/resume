...
const VerticalLayout = (props) => {
  ...
  if(auth.user?.roles[0].name == "ROLE_ADMIN"){
     menuData = props.menuData.concat(admin());
  }else{
     menuData = props.menuData.concat(user());
  }
  ...
  return (
    ...
    >
      ...
      >
        <div style={{background :'#101a33', color: "#ffff", textAlign: "center",padding: "10px",borderRadius: "5px",}}>
        <span>© ۱۴۰۳ - تمامی حقوق برای سازمان پروژه کسری خدمت محفوظ است.</span>
        <img style={{  width: "50px"}} className="fallback-logo" src={logo} alt="logo" />
        </div>
        {footer ? (
          footer
        ) : (
