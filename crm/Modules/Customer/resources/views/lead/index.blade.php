    ...
    <div x-data="multipleTable">
        ...
        <div class="panel mt-6">
            <div class="flex items-center justify-between">
                <h5 class="font-semibold text-lg">لیست لیدها</h5>
            </div>

            <div class="flex items-center justify-between btn-left space-x-2 rtl:space-x-reverse">
                <a href="{{ route('lead.create') }}"  class="btn btn-primary">
                    ایجاد لید جدید
                </a>
                <button @click="isImportModalOpen = true" class="btn btn-success">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4.5 h-4.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M7.5 7.5h-.75A2.25 2.25 0 0 0 4.5 9.75v7.5a2.25 2.25 0 0 0 2.25 2.25h7.5a2.25 2.25 0 0 0 2.25-2.25v-7.5a2.25 2.25 0 0 0-2.25-2.25h-.75m0-3-3-3m0 0-3 3m3-3v11.25m6-2.25h.75a2.25 2.25 0 0 1 2.25 2.25v7.5a2.25 2.25 0 0 1-2.25 2.25h-7.5a2.25 2.25 0 0 1-2.25-2.25v-.75" />
                    </svg>
                    ایمپورت اکسل
                </button>
                <a href="{{ route('lead.export') }}" class="btn btn-secondary">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4.5 h-4.5">
...
خروجی اکسل
                ...
                <div x-show="isDeleteModalOpen" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                    ...
                    <div class="p-5">
                        <div class="dark:text-white-dark/70 text-base font-medium text-[#1f2937]">
                            <p class="text-lg">
                                آیا مطمئن هستید که می‌خواهید لید
                                <strong class="text-primary" x-text="deletingCustomerName"></strong>
                                را حذف کنید؟ این عمل قابل بازگشت نیست.
                            </p>
                        </div>

                        <form :action="`/lead/${deletingCustomerId}`" method="POST" x-ref="deleteForm" class="hidden">
                            @csrf
                            @method('DELETE')
                        </form>
        ...
        <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
            ...
            <div class="flex items-center justify-center min-h-screen px-4"
                ...
                <div x-show="isAssignModalOpen"
                    ...
                    <!-- Header -->
                    <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                        <h5 class="font-bold text-lg">انتساب کارشناس به لید</h5>
                    ...
                    <div class="p-5">
                        ...
                        <form method="POST" action="{{ route('lead.assignUser') }}"
                              x-ref="assignForm">
                            @csrf
        ...
        <!-- Import Excel -->
        <div class="fixed inset-0 bg-black/60 z-[999] hidden overflow-y-auto" :class="isImportModalOpen && '!block'">
            <div class="flex items-center justify-center min-h-screen px-4" @click.self="isImportModalOpen = false">
                <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                    <!-- Header -->
                    <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                        <h5 class="font-bold text-lg">افزودن لیدها از طریق اکسل</h5>
                        <button type="button" class="text-white-dark hover:text-dark"
                    ...
                    <div class="p-5">
                        <form action="{{ route('lead.import') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <label class="form-label">آپلود فایل Excel</label>
                            <input type="file" name="file" accept=".xlsx,.xls" required class="form-input mb-3">
                            <div class="flex justify-end gap-2">
                                <button type="button" class="btn btn-outline-dark" @click="isImportModalOpen = false">
                                    لغو
                                </button>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            ...
            Alpine.data("multipleTable", () => ({
                ...
                init() {
                    // داده‌های لیدها از کنترلر
                    const customers = @json($leads);
                        ...
                        columns: [
                            {
                                ...
                                render: (data, cell, row) => {
                                    ...
                                    return `<div class="flex justify-center items-center gap-2">
                                        <a href="/lead/${customerId}/edit" x-tooltip="ویرایش" class="text-primary">
